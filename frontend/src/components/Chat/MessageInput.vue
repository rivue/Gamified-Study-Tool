...
<template>
  ...
  const inputVisible = computed(() => inputStore.isInputFieldVisible);
  const actionIconClass = computed(() => ({
    bounce: !actionsMenuOpen.value && !sending.value,
    active: actionsMenuOpen.value,
  }));
  const actionAvailable = computed(() => messageStore.actions.length !== 0);
  const sending = computed(() => messageStore.sending);
  
  // Watch for message length
  watch(message, (newVal) => {
    if (newVal.length > maxMessageLength) {
      popupStore.showPopup("Message is too long.");
      message.value = newVal.substring(0, maxMessageLength);
    }
  });
  ...
  const resetHeight = () => {
    nextTick(() => {
      if (messageInput.value) {
        messageInput.value.style.height = "54px";
      }
    });
  };
  ...
  const handleAction = (action: string) => {
    ...
    changeUserText(action);
    sendMessage();
  };
  
  const changeUserText = (desiredText: string) => {
    message.value = desiredText;
  };
  
  // Define component name and exposed methods (optional in setup script)
  defineExpose({
    focusTextarea
  });
  </script>
