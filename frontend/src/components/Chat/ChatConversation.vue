...
const shownIntro = ref(false);

onMounted(() => {
    eventBus.on("message-recieved", handleNewMessage);
});
...
const filteredMessages = computed(() => {
    ...
    if (filteredMsgs.length === 2) {
        ...
        // New user
        if (route.path === "/lessons" && !shownIntro.value) {
            handleNewUser();
        }
    }
...
const newToLessons = computed(() => {
    console.log(filteredMessages.value.length);
    return route.path === "/lessons" && filteredMessages.value.length === 2;
});
...
function navigateToContent(role?: string): void {
...
function isQuizMessage(msg: Message): boolean {
    let role = msg.role;
    let type = msg.type;
    return role === "assistant" && type === "quiz";
}

function isChatMessage(role: string, type?: string): boolean {
    return (
...
function isContentButton(role: string): boolean {
    return role.startsWith("challenge/") || role.startsWith("lesson/");
}

function isCompleted(role: string): boolean {
    return role.includes("?completed");
}

function isCompletionMessage(message: Message): boolean {
...
function getContentType(role: string): string {
