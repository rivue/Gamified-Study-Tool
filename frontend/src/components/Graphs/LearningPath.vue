<template>
    <div class="fixed left-8 right-8 top-0 bottom-0 overflow-hidden">
      ...
      @scroll="handleScroll"
    >
      <div class="flex items-center gap-32 px-24 min-h-[1400px] min-w-max py-24">
        <!-- Node content remains the same -->
  ...
  <script setup>
  // Script remains the same as previous version, except removed getConnectorAngle function since we no longer need it
  import { ref } from 'vue'
  import { 
    StarIcon, 
    BookOpenIcon, 
  ...
  const nodes = ref([
    {id: 1, label: 'Start Here', icon: StarIcon},

  ])
  
// New refs for drag functionality
