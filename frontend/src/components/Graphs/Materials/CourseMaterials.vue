    ...
    <div class="page-container">
        ...
        <main class="content-area">
            ...
            <div class="materials-list-section">
                <div class="tab-navigation" role="tablist" aria-label="Materials tabs">
                    <button
                        role="tab"
                ...
                <div v-if="activeTab === 'materials'" class="tab-content">
                    <div class="generate-test" v-if="isOwner">
                        <button @click="generateTest" class="action-button" :disabled="selectedMaterialIds.length === 0">
                            Generate Test
                        </button>
                    </div>
                    <ul v-if="materials.length > 0" class="materials-list">
                        <li v-for="material in materials" :key="material.id" class="material-item">
                            <input v-if="isOwner" type="checkbox" v-model="selectedMaterialIds" :value="material.id"
                                class="material-checkbox" />
                            <div class="file-info">
                                <component :is="getIconForFileType(material.type)" class="file-icon" />
                                <div class="file-details">
                                    <span class="file-name">{{ material.name }}</span>
                                    <span class="file-size">{{ formatSize(material.size) }} - Uploaded on {{
                                        material.uploadedDate }}</span>
                                </div>
                            </div>
                            <div class="file-status">
                                <span :class="['status-badge', getStatusClass(material.status)]">{{ material.status
                                    }}</span>
                            </div>
                            <div class="file-actions">
                                <button @click="showSummary(material)" class="action-button"
                                    :disabled="material.status !== 'Ready' || !material.summary">
                                    <DocumentTextIcon class="w-5 h-5" />
                                    <span>View Summary</span>
                                </button>
                                <button @click="showQuiz(material)" class="action-button"
                                    :disabled="material.status !== 'Ready' || !material.quiz || (material.quiz.questions || []).length === 0">
                ...
                <div v-if="activeTab === 'tests'" class="tab-content">
                    <ul v-if="tests.length > 0" class="materials-list">
                        <li v-for="test in tests" :key="test.id" class="material-item">
                            <div class="file-info">
                            ...
                            <div class="file-actions">
                                <button @click="showTest(test)" class="action-button"
                                    :disabled="test.status !== 'Ready' || test.questions.length === 0">
                                    <QuestionMarkCircleIcon class="w-5 h-5" />
                                    <span>Take Test</span>
                                </button>
                            </div>
        ...
        <SummaryModal v-if="summaryIsShowing" :material-name="selectedMaterial.name" :summary="selectedMaterial.summary"
            @close="closeSummaryModal" />
        <QuizModal v-if="quizIsShowing" :material-name="selectedMaterial.name"
            :questions="(selectedMaterial.quiz && selectedMaterial.quiz.questions) ? selectedMaterial.quiz.questions : []"
            @close="closeQuizModal" />
        <TestModal v-if="testIsShowing && selectedTest" :test-name="selectedTest.name"
            :questions="selectedTest.questions || []" @close="closeTestModal" />
    </div>
</template>
...
const activeTab = ref<'materials' | 'tests'>('materials');
...
const generateTest = async () => {
    ...
    try {
        await axios.post('/api/tests/generate', {
...
/* Tabs */
.tab-navigation {
    /* Center the pill and keep it sized to content */
    display: flex;
    width: max-content;
    margin: 0 auto;
    align-items: center;
